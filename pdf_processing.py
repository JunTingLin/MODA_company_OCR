...
def process_pdf_folder(folder_path, filenames, updater=None):
    """處理包含多個 PDF 和圖檔的資料夾"""
    ...
    for index, filename in enumerate(filenames):
        if filename.lower().endswith('.pdf'):
            ...
            if updater: updater.update_status(f"{filename} 已拆分...")
        else:
            updated_filenames.append(filename) # 如果不是 PDF 文件，則直接添加到列表中
            if updater: updater.update_status(f"{filename} 無需拆分...")
